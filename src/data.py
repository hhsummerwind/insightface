    ...
    def next(self):
        ...
        try:
            while i < batch_size:
                ...
                output_path = os.path.join('/data/datasets/face/recognition/ms1m-retinaface-t1/img', str(int(label)))
                if not os.path.exists(output_path):
                    os.mkdir(output_path)
                exist_img_count = len(os.listdir(output_path))
                cv2.imwrite(os.path.join(output_path, '{}.jpg'.format(exist_img_count)), _data.asnumpy()[:, :, ::-1])
                if self.rand_mirror:
                  _rd = random.randint(0,1)
                  if _rd==1:
